			...
			<table width="80%" border="1" cellspacing="0" align="center">
				<tr>
					<td colspan="5">
						<!--  
							...
							>>http://192.168.0.22:8090/WebServlet_92_Board_Model2_MVC/list.do?ps=10					
						-->
						<form name="list" >
						 PageSize설정: 
				...
				<tr>
					...
					-->
						...
						<!--이전 링크 --> 
						<c:if test="${cpage > 1}">
							<a href="list.do?cp=${cpage-1}&ps=${pagesize}">이전</a>
						</c:if>
						<!-- page 목록 나열하기 -->
						<c:forEach var="i" begin="1" end="${pagecount}" step="1">
							<c:choose>
								...
								<c:otherwise>
									<a href="list.do?cp=${i}&ps=${pagesize}">[${i}]</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<!--다음 링크 --> 
						<c:if test="${cpage < pagecount}">
							<a href="list.do?cp=${cpage+1}&ps=${pagesize}">다음</a>
						</c:if>
					</td>
